<template>
	<view class="jPicker">
		<view @click="showPicker" class="showLine">
			<view v-if="listData.length > 0 && nSel != -1 && val">{{ listData[nSel][showKey] || listData[nSel] }}</view>
			<view style="color:#000;" v-else>请选择</view>
		</view>
		...
		<view class="pickerMask" v-if="pickerVisable" @click="pickerVisable = false">
			<view class="alertArea JAnimateBtmIn" @click.stop="doNothing">
				...
				<view class="pickerTop">
					...
					<view class="rigBtn" :style="{ color: sureColor }" :data-key="dataKey" @click="sureSelect">确定</view>
				</view>
 ...
 * @event {Function} onShow 事件
 * @event {Function} onClose 事件
...
export default {
	...
	props: ['options', 'showKey', 'valKey', 'val', 'position', 'disabled', 'bgColor', 'sureColor', 'dataKey'],
	...
	methods: {
		...
		sureSelect(e) {
			let keyStr = e.currentTarget.dataset.key
			this.pickerVisable = false
